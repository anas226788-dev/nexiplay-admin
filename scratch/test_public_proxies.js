async function testProxy(name, proxyUrl, targetUrl) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (html.length < 500) {
                console.log("Response sample:", html);
            }
        } else {
...
async function run() {
    ...
    await testProxy("AllOrigins Raw", "https://api.allorigins.win/raw?url=", targetUrl);
    await testProxy("CorsProxy.io", "https://corsproxy.io/?", targetUrl);
    await testProxy("CodeTabs", "https://api.codetabs.com/v1/proxy?quest=", targetUrl);
}
