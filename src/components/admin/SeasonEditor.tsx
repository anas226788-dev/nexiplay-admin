...
export default function SeasonEditor({ movieId, movieType }: SeasonEditorProps) {
            ...
            if (markLatestUpdate) {
                try {
                    const { data: movie } = await supabase.from('movies')
                        ...
                        .single();
                    if (movie) {
                        // Preserve existing episode_number if any
                        const { data: existing } = await supabase.from('updates')
                        ...
                        const { error: upsertError } = await supabase.from('updates').upsert({
                            ...
                            update_type: existing?.episode_number ? 'episode' : 'season',
                            season_number: seasonNumber,
    ...
    const saveEpisode = async () => {
        ...
        try {
            ...
            // Latest Update upsert
            if (markLatestUpdate && selectedSeason) {
                try {
                    const { data: movie } = await supabase.from('movies')
                        .select('id, title, poster_url, slug, type')
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-dark-800 rounded-2xl border border-white/10 w-full max-w-md p-6">
                        ...
                        <div className="space-y-4">
                            ...
                            {/* Mark as Latest Update */}
                            <div className="flex items-center justify-between p-4 bg-green-900/10 rounded-xl border border-green-500/20">
                                <div>
                                    <h4 className="text-sm font-bold text-green-400">🔔 Mark as Latest Update</h4>
                                    <p className="text-xs text-gray-500">Show in homepage "Latest Updates" section</p>
                                </div>
                                <label className="relative inline-flex items-center cursor-pointer">
                                    <input
                                        type="checkbox"
                                        className="sr-only peer"
                                        checked={markLatestUpdate}
                                        onChange={(e) => setMarkLatestUpdate(e.target.checked)}
                                    />
