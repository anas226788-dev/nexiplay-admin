...
export default function MovieForm({ initialData }: MovieFormProps) {
    ...
    const [trendingBadge, setTrendingBadge] = useState(initialData?.trending_badge || false);
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const url = await uploadPoster(file, title);
    ...
    const handleScreenshotUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const uploadPromises = files.map(file => uploadPoster(file, title ? `${title}-screenshot` : undefined));
    ...
    const handleBannerUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'desktop' | 'mobile') => {
        ...
        setUploading(true);
        const file = e.target.files[0];
        const url = await uploadPoster(file, title ? `${title}-banner-${type}` : undefined);
        ...
        <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-8">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        {isRunning && (
                            <div className="space-y-4 pt-4 border-t border-white/5 animate-in fade-in width-full">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Admin Note (Private)</label>
                                        <textarea
                                            ...
                                            placeholder="Internal notes... (Hidden from users)"
                                        />
                                    </div>
                                    <div className="flex items-end pb-4">
                                        <label className="flex items-center gap-3 p-3 bg-dark-900 w-full rounded-lg border border-white/10 cursor-pointer hover:bg-dark-800 transition-colors">
                                            <input
                                                type="checkbox"
                                                checked={notifyAdmin}
                                            ...
                                            />
                                            <span className="text-white font-medium text-sm">Flag for Admin Attention 🔔</span>
                                        </label>
                                    </div>
                                </div>
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-bold flex items-center gap-2">
                            <span>🔞</span> Mark as Adult (18+)
                        </h2>
                        <p className="text-sm text-gray-400 mt-1">
                            Show an &quot;18+&quot; badge and require age verification before users can access this content.
                        </p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                        <input
            ...
            <div className="glass p-6 rounded-xl border border-white/5 space-y-2">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-bold flex items-center gap-2">
                            <span className="text-yellow-500">⭐</span> Mark as Trending
                        </h2>
                        <p className="text-sm text-gray-400 mt-1">
                            Show a highlighted &quot;TRENDING&quot; badge on this content&apos;s poster card. If OFF, a &quot;NEW&quot; badge will show instead.
                        </p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                        <input
                            type="checkbox"
                            className="sr-only peer"
                            checked={trendingBadge}
                            onChange={(e) => setTrendingBadge(e.target.checked)}
                        />
