...
export default function AddAdPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (formData.ad_type === 'image' && (!formData.image_url.trim() || !formData.destination_url.trim())) {
            setError('Image URL and Destination URL are required for image ads.');
            setLoading(false);
            return;
        }
        ...
        try {
            const { error: insertError } = await supabase
                ...
                .insert([{
                    ...
                    // Set safe default size to avoid constraints (optional)
                    size: 'auto'
                }]);
    ...
    return (
        <div className="max-w-2xl mx-auto">
            <div className="flex items-center gap-4 mb-8">
                ...
                <h1 className="text-2xl font-bold text-white font-outfit">Create New Ad</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="glass-panel p-8 rounded-xl space-y-6">
                ...
                <div className="pt-4">
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full py-4 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl transition-all shadow-lg shadow-red-900/30 disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                        {loading ? (
                            <>
