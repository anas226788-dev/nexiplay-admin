                        ...
                        requests.map((req) => (
                            <div key={req.id} className="glass-panel rounded-xl p-4 space-y-3">
                                <div className="flex items-start justify-between gap-2">
                                    <h3 className="font-medium text-white">{req.content_name}</h3>
                                    <span className={`px-2 py-1 text-xs font-bold rounded capitalize whitespace-nowrap ${req.status === 'added' ? 'bg-green-500/20 text-green-400' :
                                        req.status === 'rejected' ? 'bg-red-500/20 text-red-400' :
                                            'bg-yellow-500/20 text-yellow-400'
                                        }`}>
                                        {req.status}
                                ...
                                <div className="flex items-center gap-2 pt-2 border-t border-white/5">
                                    {req.status === 'pending' && (
                                        <button
                                            onClick={() => updateStatus(req.id, 'added')}
                                            className="flex-1 py-2 bg-green-500/20 text-green-400 rounded-lg text-xs font-bold active:bg-green-500/30 transition-colors"
                                        >
                                            ✓ Mark Added
                                        </button>
                                    )}
                                    <button
                                        onClick={() => deleteRequest(req.id)}
                                        className="flex-1 py-2 bg-red-500/20 text-red-400 rounded-lg text-xs font-bold active:bg-red-500/30 transition-colors"
                                    >
                                        Delete
                                    </button>
                                </div>
