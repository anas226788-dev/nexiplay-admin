                ...
                <div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button
                            onClick={runScan}
                            disabled={loading}
                            className="text-xs px-3 py-1 glass-button bg-red-600/80 hover:bg-red-700 disabled:opacity-50 rounded text-white font-bold transition-colors"
                        >
                            {loading ? 'Scanning...' : 'Run Scan (Batch 10)'}
                        </button>
                    </div>
                </div>
                    ...
                    <>
                        ...
                        <div className="md:hidden space-y-4 p-4">
                            {deadLinks.map((item) => {
                                const expired = getExpiredProviders(item.link_status);
                                return expired.map((prov) => (
                                    <div key={`${item.id}-${prov}`} className="glass-panel rounded-xl p-4 space-y-3">
                                        ...
                                        <div className="bg-red-500/5 rounded-lg p-3 border border-red-500/10">
                                            <div className="flex items-center gap-2 text-red-400 font-bold uppercase text-xs mb-1">
                                                <span className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse"></span>
                                                {prov} Broken
                                            </div>
                                            <a href={item[prov + '_link']} target="_blank" className="text-xs text-gray-500 hover:text-white underline truncate block">
                                                {item[prov + '_link']}
                                            </a>
                                        </div>
                                        ...
                                        <button
                                            onClick={() => markActive(item.id, prov + '_link', item.link_status)}
                                            className="w-full py-2.5 bg-green-600/20 text-green-400 text-xs font-bold rounded-lg hover:bg-green-600/30 active:scale-95 transition-all"
                                        >
                                            ✅ Mark Active
                                        </button>
                                    </div>
